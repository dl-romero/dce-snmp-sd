...
Prometheus [HTTP service discovery](https://prometheus.io/docs/prometheus/latest/http_sd/) server built for Schneider Electric [Data Center Expert (DCE)](https://www.se.com/us/en/product-subcategory/4140-data-center-expert/) environments.

Maintains a live SNMP device inventory — populated automatically from one or more DCE servers via the DCE SOAP API, or managed manually via REST API. When a device is added, the service probes it over SNMP to resolve its hostname and the correct `snmp_exporter` module (using `module_lookup.json` from [dce-snmp-modules](https://github.com/dl-romero/dce-snmp-modules)). Hostnames are re-checked every 6 hours so the `/targets` endpoint stays current as devices change.
...
| Repo | Role |
|---|---|
| **[dce-snmp-modules](https://github.com/dl-romero/dce-snmp-modules)** | Converts Schneider Electric / APC DDF files into `snmp.yml` and `module_lookup.json` for `snmp_exporter` |
| **dce-snmp-sd** *(this repo)* | Pulls device inventory from DCE servers via SOAP, resolves SNMP modules automatically, serves a live `/targets` endpoint for Prometheus |

Run both together: `dce-snmp-modules` generates the config files that `dce-snmp-sd` uses for module resolution and that `snmp_exporter` uses for SNMP walks.
...
```
┌─────────────────┐  SOAP (hourly)   ┌──────────────────┐  http_sd  ┌────────────┐
│ Data Center     │ ────────────────▶│                  │ ─────────▶│ Prometheus │
│ Expert (DCE)    │                  │   dce-snmp-sd    │           └─────┬──────┘
└─────────────────┘                  │                  │                 │ scrape
                                     │                  │           ┌─────▼──────┐
┌─────────────────┐   REST API       │                  │           │snmp_exporter│
│  You / ops      │ ────────────────▶│                  │           └─────┬──────┘
└─────────────────┘   add/remove     └────────┬─────────┘                │
                                              │ SNMP probe           ┌───▼────────┐
                                              └─────────────────────▶│  Devices   │
...
1. DCE servers are added via `POST /api/dce` — the service fetches the full device list from DCE's SOAP API every hour, adding new devices and removing ones that are no longer in DCE
2. Devices can also be added manually via `POST /api/devices` for non-DCE-managed targets
3. When a device is added (from either source), it is immediately probed via SNMP — fetching `sysName`, `sysObjectID`, and `sysDescr`
4. `sysObjectID` is matched against the DDF lookup index to find the right `snmp_exporter` module
5. The device appears in `/targets` with `__param_module` and `__param_auth` labels set
6. Prometheus polls `/targets` and routes scrapes through `snmp_exporter`
7. Every 6 hours the service re-probes all devices and updates hostnames if they change
...
- Python 3.10+
- [`dce-snmp-modules`](https://github.com/dl-romero/dce-snmp-modules) — provides `output/module_lookup.json` and `output/snmp.yml`
...
│   ├── dce_store.py       # JSON-backed async DCE server store
│   ├── dce_client.py      # DCE SOAP client (ISXCentralDeviceService_v2_0)
│   ├── dce_sync.py        # Hourly DCE inventory sync scheduler
│   ├── snmp_discovery.py  # SNMP probing + module resolution
│   └── scheduler.py       # Background hostname refresh loop
├── wsdl/                  # Official DCE web services WSDL files (reference)
│   ├── ISXCentralDeviceService_v2_0.wsdl
│   ├── ISXCentralSensorService_v2_0.wsdl
│   ├── ISXCentralAlarmsService_v2_0.wsdl
│   └── ISXCentralDeviceGroupService_v2_0.wsdl
├── systemd/
│   └── dce-snmp-sd.service
├── Dockerfile
├── docker-compose.yml
...
```bash
git clone https://github.com/dl-romero/dce-snmp-sd.git
cd dce-snmp-sd
...
|---|---|---|
| `--lookup PATH` | `/opt/dce-snmp-modules/output/module_lookup.json` | Path to DDF module lookup index |
...
```bash
sudo bash install.sh \
  --lookup /data/dce-snmp-modules/output/module_lookup.json \
  --port 9200
```
...
```
/opt/dce-snmp-sd/           app files + venv
/var/lib/dce-snmp-sd/       devices.json persistent store
/etc/systemd/system/dce-snmp-sd.service
```

A `dce-snmp-sd` system user is created with ownership of the data directory.

### Managing the service

```bash
sudo systemctl status dce-snmp-sd
sudo journalctl -u dce-snmp-sd -f
sudo systemctl restart dce-snmp-sd
```
...
```bash
sudo systemctl edit dce-snmp-sd
```

| Variable | Default | Description |
|---|---|---|
| `MODULE_LOOKUP_PATH` | `/opt/dce-snmp-modules/output/module_lookup.json` | DDF module lookup index |
| `DATA_FILE` | `/var/lib/dce-snmp-sd/devices.json` | Device inventory persistence |
...
dce-snmp-sd can automatically populate its device inventory by pulling from one or more Schneider Electric Data Center Expert servers. Engineers add server credentials once; the service syncs the full device list every hour.
...
The WSDL files for all four services are kept in the `wsdl/` directory for reference. Authentication is **HTTP Basic Auth** sent with every request — no session management required. The `host` field accepts `hostname`, `IP address`, `http://hostname`, or `https://hostname`. If no scheme is given, `https://` is assumed.
...
[
  {
    ...
    "source": "dce:a1b2c3d4",
    "labels": {"location": "dc1", "rack": "A01", "dce_server": "DC1 DCE"},
...
```json
{
  ...
  "config": {
    ...
    "dce_sync_interval_hours": 1.0,
    "snmp_timeout": 3,
    "module_lookup": "/opt/dce-snmp-modules/output/module_lookup.json"
  }
}
```
...
mkdir -p data
cp /path/to/dce-snmp-modules/output/module_lookup.json data/
cp /path/to/dce-snmp-modules/output/snmp.yml data/

# 2. Configure (optional — all have defaults)
cp .env.example .env
# Edit .env if you want a different port, refresh interval, etc.

# 3. Start both dce-snmp-sd and snmp_exporter
docker compose up -d
```
...
| `PORT` | `8000` | Host port for dce-snmp-sd |
| `REFRESH_INTERVAL_HOURS` | `6` | How often to re-probe all devices for hostname changes |
| `DCE_SYNC_INTERVAL_HOURS` | `1` | How often to sync device inventory from DCE servers |
| `SNMP_TIMEOUT` | `3` | SNMP timeout per device in seconds |
| `SNMP_RETRIES` | `1` | SNMP retries per device |
...
If you're running `dce-snmp-modules`'s sync service on the same host with the data directory bind-mounted, you can add this call to your sync script to make the reload automatic.

### Build the image locally

```bash
docker build -t dce-snmp-sd:latest .
```
...
Run this on the `dce-snmp-modules` host, then reload without restarting:
```bash
python3 /opt/dce-snmp-modules/scripts/build_lookup.py \
  /opt/Schneider-Electric_SNMP-DDF-Downloader/ddf_files \
  -o /opt/dce-snmp-modules/output/module_lookup.json

# Reload without restart (systemd or Docker)
curl -X POST http://localhost:8000/api/reload-lookup
```

If you're using the daily sync service from `dce-snmp-modules`, the lookup is regenerated automatically every night.
